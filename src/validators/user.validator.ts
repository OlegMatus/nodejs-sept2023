...
export class UserValidator {
  private static userName = joi.string().min(3).max(40).trim().messages({
    "string.empty": "{{#label}} not be empty",
    "string.min": "{{#label}} must be at least {{#limit}} characters long2",
    "string.max":
  ...
  private static age = joi.number().min(15).max(100);
